...
exports.user_detail = function (req, res) {
  ...
  User.getTodoById(id, (err, post) => {
    if (err) res.json({ success: false, 'response_code': 400, 'message': 'error', 'result': 'Something went wrong.' })
    if (post) { res.json({ success: true, 'response_code': 200, 'message': 'success', 'result': post }) }
    else { res.json({ success: false, 'response_code': 400, 'message': 'error', 'result': 'Invalid Todo Identity.' }) }
  })
}
